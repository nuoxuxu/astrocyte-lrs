...
process RUN_RIBOTIE {
    ...
    input:
    tuple val(param_set_name), path(gtf_h5), path(ribotie_h5), val(mode), path(ribotie_yml)

    output:
    tuple val(param_set_name), path("ribotie_res_${mode}.redundant.gtf"), emit: redundant_gtf
    tuple val(param_set_name), path("ribotie_res_${mode}.redundant.csv"), emit: redundant_csv
    tuple val(param_set_name), path("ribotie_res_${mode}.novel.gtf"),     emit: novel_gtf
    tuple val(param_set_name), path("ribotie_res_${mode}.novel.csv"),     emit: novel_csv
    tuple val(param_set_name), path("ribotie_res_${mode}.gtf"),           emit: filtered_gtf
    tuple val(param_set_name), path("ribotie_res_${mode}.csv"),           emit: filtered_csv
    tuple val(param_set_name), path("ribotie_res_*.ckpt"), path("ribotie_res_*.npy"), path("ribotie_res_*.yml"), path("models"), path("multiqc"), emit: other
