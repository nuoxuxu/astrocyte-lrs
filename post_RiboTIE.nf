...
include { ISOFORMSWITCH as ISOFORMSWITCH_all2 } from "./subworkflows/local/IsoformSwitchAnalyzeR/main.nf"
...
workflow {
    ...
    ISOFORMSWITCH_all2(channel.value("ALL2"), prepare_supplemented_files.out.supplemented_expression, primer_to_sample, prepare_supplemented_files.out.supplemented_fasta, supplement_collaborator_gtf.out.supplemented_gtf, annotation_gtf, final_classification)

    AIM_2(supplement_collaborator_gtf.out.supplemented_gtf, annotation_gtf, bigbrain_sqtl, bigbrain_coloc, ISOFORMSWITCH_all2.out.isoform_features_csv, leafcutter_sig, leafcutter_clu2gene)

    FILTER_RIBOTIE(collaborator_gtf, final_fasta, final_expression, final_classification, ref_genome_fasta)

    ISOFORMSWITCH_ORF(channel.value("ORF"), FILTER_RIBOTIE.out.filtered_RiboTIE_expression, primer_to_sample, FILTER_RIBOTIE.out.filtered_RiboTIE_fasta, collaborator_gtf, annotation_gtf, FILTER_RIBOTIE.out.filtered_RiboTIE_classification)
